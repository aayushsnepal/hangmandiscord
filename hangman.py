async def hangman(message):
    ...
    user = message.author
    r = RandomWords()
    words = str(r.get_random_word(maxLength=8))
    wo = words
    word = list(words)
    w = ""
    for x in word:
    ...
    while not gameOver and attempts > 0:
        ...
        try:
            ...
            if guess.author == message.author and len(guess.content) != 1 and guess.content != "quit" and \
                    guess.content != wo:
                await channel.send("One letter at a time.")
            else:
                if guess.author == message.author:
                    if guess.content == "quit":
                        await channel.send("Game exited.")
                        return
                    elif guess.content == wo:
                        gameOver = True
                        await channel.send("You guessed the word! The word was: `" + w + "`")
                    elif guess.content in word and guess.content not in correctguesses:
                        await channel.send("A correct letter!")
                        correctguesses.append(guess.content)
                        x = 0
                        for i in word:
                            if i == guess.content:
                                current[x * 2] = word[x]
                            x += 1
                    elif guess.content in correctguesses:
                        await channel.send("`" + guess.content + "` has already been guessed **correctly**!")
                    elif not gameOver:
                        if guess.content not in wrongguesses:
                            wrongguesses.append(guess.content)
                            attempts -= 1
                            await channel.send("Wrong guess. Attempts left: " + str(attempts))
                        elif guess.content in wrongguesses:
                            await channel.send(
                                "`" + guess.content + "` has already been guessed **incorrectly**! Guess again.")
                    if not gameOver:
                        st = "`"
                        for e in current:
                            st += e
